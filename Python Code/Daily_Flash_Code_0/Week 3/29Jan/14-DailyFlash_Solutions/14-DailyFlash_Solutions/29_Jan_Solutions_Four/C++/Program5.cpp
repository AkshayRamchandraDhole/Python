...
int main() {
	...
	do {
		...
		if(date <= 0 || date > 31){
			std::cout<<"Invalid Input, Month cannot be greater than 12 or negative"<<std::endl;
		}

		//If statement to print error message if Condition 1 or Condition 3 is false
		if(month <= 0 || month > 12){

			std::cout<<"Invalid Input, Month cannot be greater than 31 or negative"<<std::endl;
		}
		//If statement to print error message if Condition 1 or Condition 2 is false
		if(year <= 0 || year > 9999){

			std::cout<<"Invalid Input, Month cannot be greater than 9999 or negative"<<std::endl;
		}

	}while(((month <= 0 || month > 12) || (date <= 0 || date > 31) || (year <= 0 || year > 9999)) && !std::cin.fail());
	...
	if (month == 2){
		...
		if (date <= 28){
			...
			std::cout<<"The Date "<<date<<"/"<<month<<"/"<<year<<" is valid"<<std::endl;
		}
		...
		//All the rest dates for february are invalid
			...
			std::cout<<"The Date "<<date<<"/"<<month<<"/"<<year<<" is Invalid"<<std::endl;
		}
	}
